...
在传统工作流中，在全局布线的第一阶段为树生成阶段，此阶段通常采用FLUTE算法生成树拓扑，因此传统算法中通常以线长最优为主导，较少考虑拥塞信息，导致前期候选解空间偏窄。当模板布线阶段再叠加有限模板搜索时，这种偏差会进一步放大，最终将大量高难度线网推入撤销重布与迷宫布线阶段，造成运行时间显著上升，并使后期修复空间受到限制。针对该问题，本文将研究重点前移到初始布线阶段，构建可感知拥塞、可迭代更新、可并行求解的初始布线工作流。

\subsection{方法与贡献}
针对上述问题，本文将研究重点前移至初始布线阶段，构建动态拥塞感知初始布线工作流，核心贡献如下：
\begin{enumerate}
    \item 提出拥塞感知的候选树森林构建机制，在初始化阶段同时保留线长导向与拥塞感知导向两类树拓扑，为后续联合优化提供更有区分度的候选空间；
    \item 将面向并行平台的求解流程集成至初始布线工作流中，将数据级并行与线网级并行协同嵌入求解过程，在控制质量退化风险的前提下提升整体效率；
    \item 提出动态更新树森林与规模缩减机制，在多轮训练之间分别处理拥塞线网与非拥塞线网，使优化资源持续聚焦于真正的瓶颈区域。
\end{enumerate}
...
该策略本质上是在初始化阶段获得不同指标优化导向的候选：RSMT候选偏线长友好，绕线树候选偏拥塞友好。
...
\begin{equation}
    \tau^{(r+1)}=0.9\tau^{(r)}
\end{equation}
逐轮退火，实现前期探索、后期收敛的离散化逼近。上述温度参数设置参考了可微全局布线方法DGR的经验配置：初始值$\tau^{(0)}=1.0$可保证训练初期候选概率分布较为平滑，保留充分的探索空间；衰减系数0.9的设置使得温度在5轮迭代后降至约0.59，10轮后降至约0.35，既避免前期过早收敛到局部最优，也保证后期概率分布足够尖锐以逼近离散决策。若设计规模显著增大或拥塞程度更高，可适当调整温度设置与衰减率，以平衡探索与收敛速度。
...
由于上述联合目标表达式，可以根据实际优化偏好设置不同的权重参数 $\lambda_{of},\lambda_{wl},\lambda_{via}$，以实现不同的优化导向（例如更强调拥塞修复或更强调线长优化）。当前版本中，本文采取CUGR2的权重参数设置，具体设置为 $\lambda_{of}=500$、$\lambda_{wl}=0.5$、$\lambda_{via}=2$，该设置参考了全局布线基准方法CUGR2的经验配置：$\lambda_{of}=500$ 使溢出惩罚在总损失中占主导地位，确保初始布线阶段优先缓解拥塞；$\lambda_{wl}=0.5$ 和 $\lambda_{via}=2$ 则控制线长与通孔指标的退化幅度，实现多目标平衡。若需调整优化偏好，可适当提高 $\lambda_{wl}$ 至1.0以更关注线长，或提高 $\lambda_{via}$ 至3.0以控制通孔数量，但需注意避免过度降低 $\lambda_{of}$ 导致拥塞优化效果下降。
...
在拥塞感知树生成环节，核心不是直接替换全部候选，而是先通过试布线得到拥塞视图，再仅对拥塞线网执行候选更新。边移动机制在此阶段通过局部绕行改变树边穿越位置，其作用是生成更拥塞友好的候选树，而不是独立的后处理步骤。图\ref{fig:chap3-edge-shift-placeholder}给出了该局部操作的几何示意，图\ref{fig:chap3-two-tree-example-placeholder}展示了线长友好候选与拥塞友好候选的对比结果。
...
本节实验围绕两个问题展开：所提动态拥塞感知初始布线流程是否能有效降低拥塞线网数量与溢出，以及在引入动态更新与动态缩减后算法能否在可接受开销内取得稳定收益。
...
\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.80\textwidth]{image/chap03/top001_hotspot_variance_normalized_barh.pdf}
...
本章围绕拥塞感知初始布线工作流，系统介绍了从候选树森林构建、可微解析建模，再到树森林动态更新与规模缩减策略，并详细介绍了技术实现细节。与DGR等可微布线方法相比，本文方法通过更新拥塞线网候选 + 缩减非拥塞线网训练集合的闭环策略，使优化过程具备持续反馈能力，能够在线长、通孔数指标维持稳定的前提下显著降低溢出相关指标。

实验结果表明，本文流程在31个公开基准上取得稳定改进：拥塞指标相比DGR降低24.9\%、相比CUGR2降低44.1\%，线长与通孔指标近乎无退化，在可控计算开销下实现了质量与效率的协同增益。以上结论为后续章节的进一步分析与拓展提供了方法基础。
