...
第三章已经围绕动态拥塞感知初始布线给出了质量与效率协同优化方案。本章工作进一步向上游延伸，研究如何在已有模块级布局规划的基础上，通过外层设计空间探索（Design Space Exploration, DSE）在布局阶段就获得一定的质量改善收益。该思路通过低侵入方式对模块级布局规划结果进行细粒度调整，与既有布局优化算法耦合度低，可较容易插入到现有模块级布局算法中，使后续布线在较优的布局结果上运行。
...
集成电路布局本质上是多约束、多目标、强耦合优化问题。其典型优化目标为性能-功耗-面积（Performance, Power, Area, PPA），同时必须还要满足工艺可制造性和设计规则。该问题已被广泛认为具有非确定性多项式困难（Non-deterministic Polynomial-time hard, NP-hard）属性问题。
...
邻域操作决定了模拟退火在可行空间中的探索方式。考虑到\ref{sec:chap4-legalization}中给出的合法化约束，本文要求每次扰动后得到的新约束尽可能直接满足工具可接受的曼哈顿化表达，避免因额外二次合法化引入不可控偏差。基于该原则，本文将邻域定义为如下五类原子操作：
\begin{equation}
...
实验结果表明，在可接受运行开销下，该流程在多个测试样例上实现了一定的线长优化；同时，案例可视化、退火迭代曲线与密度统计分析三方面结论相互印证，本文方法的收益来自模块级约束引导的细粒度调整与多段退火搜索的协同作用。上述结果验证了本章方法在布局结果质量上改进的有效性。
